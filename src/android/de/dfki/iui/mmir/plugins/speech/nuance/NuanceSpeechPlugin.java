...
//		}
		...
		cordova.getThreadPool().execute(new Runnable() {
            public void run() {
                ...
                NuanceEngine.getInstance().recognize(recognizer, doUseShortPauseDetection, doUseDictationLanguageModel, false);
            }
        });
		...
		if (withIntermediateResults){
			...
			cordova.getThreadPool().execute(new Runnable() {
				public void run() {
					...
					NuanceEngine.getInstance().recognize(recognizer, doUseShortPauseDetection, doSuppressStartPrompt, doUseDictationLanguageModel);
				}
			});
