...
public class NuanceSpeechPlugin extends CordovaPlugin {
	...
	private boolean initNuanceEngine() {
		return this.initNuanceEngine(this.cordova.getActivity(), this.preferences);
	}

	private boolean initNuanceEngine(Context context, CordovaPreferences prefs) {
		boolean success = NuanceEngine.createInstance(context, prefs);
		if (success)
			LOG.d(PLUGIN_NAME, "(re-)initialized");
		else
			LOG.d(PLUGIN_NAME, "could not initialize (credentials may be missing)");
	 ...
	 *
	 * @param doEnableWakeLock if <code>true</code> the Window is set with FLAG_KEEP_SCREEN_ON,
	...
	public boolean execute(String action, JSONArray data, CallbackContext callbackContext) {
		...
		if(!initNuanceEngine()){
			...
			//some actions require full initialization of the NuanceEngine (i.e. valid session etc):
			if(TTS.equals(action) || ASR.equals(action) | ASR_NO_EOS_DETECTION.equals(action) || ASR_SHORT_PAUSE.equals(action)){
				
				callbackContext.sendPluginResult(createNotInitializedError(action));
	 ...
	 *
	 * @param data          the JSON argument data
	 * @param index         the index within the JSON argument data, which contains the language parameter
	 * @param actionContext the action's name (i.e. during which action the language is set) - FOR DEBUGGING / ERROR LOG
	 * @return boolean <code>true</code> if the language was set
	 */
	...
	private PluginResult createNotInitializedError(String action) {
		return new PluginResult(PluginResult.Status.ERROR, PLUGIN_NAME+"."+action+": not initialized yet, credentials may be missing");
	}
	 ...
	 *
	 * @param data            additional data / arguments (CURRENTLY NOT USED)
	 * @param callbackContext the callback context (JavaScript side) for results
	 * @return a NO_RESULT PluginResult (set to keepCallback)
	 */
	private PluginResult recognize(JSONArray data, final CallbackContext callbackContext) {

		//DISABLED: invalid option for recognize (only for startRecord)
	 ...
	 * @param callbackContext         the callback context (JavaScript side) for results
	 * @param withIntermediateResults if <code>true</code>, intermediate recognition results will be returned on the callbackContext
	...
	class ExtendedRecognizerListener extends Transaction.Listener { //implements Recognition.Listener {
		...
		private static final String JS_PLUGIN_ID = "mmir-plugin-speech-nuance.nuanceSpeechPlugin";
